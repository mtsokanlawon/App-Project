...
decoded_prediction1 = decoding_dict[prediction1[0]]
decoded_prediction2 = decoding_dict[prediction2[0]]
# use the model to predict

prediction_proba = model.predict_proba(input_df)
prediction_proba_df = pd.DataFrame(prediction_proba)
prediction_proba_df.rename(columns = decoding_dict, inplace=True)
...
def data_selection(df):
    ...
    while selection_dict == null_dict:
        selection_dict = {
            "rows":[0],
            "columns":[],
            }
...
st.write('**Selected Test Data** -Default(First Row)')
st.write(selection_df)
...
prediction = model.predict(selection_df)
prediction0 = model2.predict(selection_df)
 
decoded_prediction = decoding_dict[prediction[0]]
decoded_prediction0 = decoding_dict[prediction0[0]]

prediction_prob = model.predict_proba(selection_df)
prediction_prob_df = pd.DataFrame(prediction_prob)
prediction_prob_df.rename(columns = decoding_dict, inplace=True)
